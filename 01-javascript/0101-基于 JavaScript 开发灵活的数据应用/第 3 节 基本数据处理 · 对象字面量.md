...
<mark>虽然使用 `const` 语句所定义的对象是不能直接被替换的，但是其中的内容依然能被修改。</mark>

栈是一种线性数据结构，类似于一个临时存储器，用于存储局部变量和方法调用。栈通常用于存储基本数据类型（例如，整数、字符、布尔值等）和指针（例如，对象的引用）。

堆是一种较复杂的数据结构，用于存储动态分配的内存。它支持更灵活的分配和释放，由开发者手动进行操作。堆内存通常用于存储对象和数组等复杂数据结构。你通过new关键字创建一个对象或者数组时，实际的数据存储在堆内存中，而对这个数据的引用（即指针）存储在栈上。
